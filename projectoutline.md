...
| Features | `src/app/features/*` | One folder per domain (`marketing`, `auth`, `patients`, `appointments`, `emr`, `billing`, `admin`, `analytics`, `dashboard`, `settings`), each with its own routes, services, and typed request/response models matching the backend's Express routes exactly |

**Public site:** `/` serves `features/marketing` — a single-page marketing site (no auth required) with its own dark "console" visual language distinct from the authenticated app: sticky nav, animated hero with a live-activity console panel, animated metric counters, a capability grid, a real 4-step product walkthrough, role-based value props, a security/compliance section, testimonials, pricing, FAQ, and a closing CTA. Its signature element is `PulseLineComponent`, a continuous animated ECG trace used as a structural motif (hero → "how it works" spine → closing CTA) rather than as decoration. Motion is implemented with two shared directives — `ScrollRevealDirective` (IntersectionObserver-based reveal-on-scroll) and `CountUpDirective` (animated counters) — both of which no-op under `prefers-reduced-motion`. Authenticated visitors who land on `/` see a "Go to dashboard" CTA in the nav instead of "Sign in."
