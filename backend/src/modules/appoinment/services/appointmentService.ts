...
export class AppointmentService {
  ...
  static async createAppointment(
    ...
    if (data.startTime < schedule.startTime || 
      ...
      throw new BadRequestError(
        `Doctor is only available from ${schedule.startTime} to ${schedule.endTime}`
      );
    }
    ...
    await this.checkSchedulingConflict(
      ...
      data.startTime,
      endTime,
      undefined // exclude current appointment
    );

    // Check patient doesn't have another appointment at the same time
    await this.checkPatientConflict(
      data.patientId,
      data.appointmentDate,
      data.startTime,
      endTime
    ...
    if (appointmentsCount >= maxPatients) {
      throw new BadRequestError('Doctor has reached maximum patients for this day');
    }
    ...
    this.sendAppointmentNotifications(appointment, 'created').catch((error) => {
      logger.error('Failed to send appointment notifications:', error);
    });
   ...
   * ============================================
   */
  static async listAppointments(query: AppointmentQueryInput): Promise<AppointmentListResponse> {
    const {
      page = 1,
      limit = 10,
      patientId,
  ...
  static async updateAppointment(
    ...
    if (['COMPLETED', 'CANCELLED', 'NO_SHOW'].includes(existingAppointment.status)) {
      throw new BadRequestError(`Cannot update ${existingAppointment.status.toLowerCase()} appointment`);
    }
  ...
  static async rescheduleAppointment(
    ...
    if (!['SCHEDULED', 'CONFIRMED'].includes(existingAppointment.status)) {
      throw new BadRequestError('Only scheduled or confirmed appointments can be rescheduled');
    }
  ...
  static async cancelAppointment(
    ...
    if (['COMPLETED', 'CANCELLED'].includes(existingAppointment.status)) {
      throw new BadRequestError('Appointment is already ' + existingAppointment.status.toLowerCase());
    }
  ...
  private static async checkSchedulingConflict(
    ...
    if (conflictingAppointment) {
      throw new ConflictError(
        `Time slot ${startTime}-${endTime} conflicts with existing appointment (${conflictingAppointment.appointmentId})`
      );
    }
  }

  private static async checkPatientConflict(
    patientId: string,
    date: string,
    startTime: string,
    endTime: string
    ...
    if (conflictingAppointment) {
      throw new ConflictError(
        `Patient already has an appointment at this time (${conflictingAppointment.appointmentId})`
      );
    }
  }

  private static async sendAppointmentNotifications(
    appointment: any,
    action: string
  ): Promise<void> {
    ...
    if (patient.email) {
      ...
      EmailService.sendMail(patient.email, subject, html).catch((err) => {
        logger.error('Failed to send patient email:', err);
      });
    }
    ...
    if (patient.phone) {
      ...
      SmsService.sendSms(patient.phone, message).catch((err) => {
        logger.error('Failed to send patient SMS:', err);
      });
    }
    ...
    if (doctor.email) {
      ...
      EmailService.sendMail(doctor.email, subject, html).catch((err) => {
        logger.error('Failed to send doctor email:', err);
      });
    }
  }

  private static getNotificationSubject(action: string): string {
  ...
  private static getSmsMessage(data: any, action: string): string {
  ...
  private static getAppointmentInclude(): Prisma.AppointmentInclude {
    return {
      patient: {
        select: {
          id: true,
  ...
  private static formatAppointmentResponse(appointment: any): AppointmentResponse {
    return {
      id: appointment.id,
      appointmentId: appointment.appointmentId,
      patient: {
