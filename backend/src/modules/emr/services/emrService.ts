    ...
    const emr = await prisma.$transaction(async (tx) => {
      const newEMR = await tx.medicalRecord.create({
        data: {
          ...
          symptoms: data.symptoms || Prisma.JsonNull,
          vitalSigns: data.vitalSigns || Prisma.JsonNull,
    ...
    const emr = await prisma.$transaction(async (tx) => {
      const updated = await tx.medicalRecord.update({
        ...
        data: {
          ...
          ...(data.symptoms !== undefined && { symptoms: data.symptoms ?? Prisma.JsonNull }),
          ...(data.vitalSigns !== undefined && { vitalSigns: data.vitalSigns ?? Prisma.JsonNull }),
