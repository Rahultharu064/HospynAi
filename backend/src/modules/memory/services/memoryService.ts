...
export class MemoryService {
  ...
  static async saveMemory(
    ...
    logger.info(`Memory saved: ${memory.id} (${data.memoryType})`);
   ...
   * ============================================
   */
  static async searchMemories(data: SearchMemoryInput): Promise<MemorySearchResponse> {
    const startTime = Date.now();

    // Build filter for database query
    const where: any = {};
    if (data.userId) where.userId = data.userId;
    if (data.patientId) where.patientId = data.patientId;
    if (data.memoryType) where.memoryType = data.memoryType;
    ...
    const results: MemorySearchResult[] = memories.map((m) => {
      ...
      return {
        ...
        relevanceScore,
        matchedTags: this.findMatchingTags(data.query, (m.metadata as any)?.tags || []),
        highlightSnippet,
      };
    });
  ...
  static async getMemoryById(id: string): Promise<MemoryResponse> {
    const memory = await prisma.aiMemory.findUnique({ where: { id } });
  ...
  static async updateMemory(
    ...
    if (data.importance !== undefined) updateData.relevanceScore = data.importance;
    if (data.tags) {
      updateData.metadata = {
   ...
   * ============================================
   */
  static async deleteMemory(id: string, userId: string): Promise<void> {
    const memory = await prisma.aiMemory.findUnique({ where: { id } });
    if (!memory) throw new NotFoundError('Memory not found');
   ...
   * ============================================
   */
  static async listMemories(query: MemoryQueryInput): Promise<MemoryListResponse> {
   ...
   * ============================================
   */
  static async getPatientContext(patientId: string): Promise<PatientContextMemory> {
  ...
  static async consolidateMemories(
    ...
    const memories = await prisma.aiMemory.findMany({ where });
    ...
    const analysis = await gptClient.generateResponse(
      `Please analyze and summarize these patient memories:\n\n${combinedContent}\n\nProvide: 1) Summary 2) Key insights 3) Recommendations`,
      'SUMMARIZE_RECORDS',
      {}
    );
    ...
    logger.info(`Memories consolidated: ${memories.length} → 1`);
  ...
  private static findMatchingTags(query: string, tags: string[]): string[] {
  ...
  private static formatMemoryResponse(memory: any): MemoryResponse {
    return {
      id: memory.id,
      ...
      source: (memory.metadata as any)?.source || null,
      sessionId: (memory.metadata as any)?.sessionId || null,
      accessCount: 0,
      lastAccessedAt: null,
